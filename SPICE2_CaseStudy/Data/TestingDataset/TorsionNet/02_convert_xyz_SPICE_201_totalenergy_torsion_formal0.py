...
for split in ["dimers"]:
    infile = h5py.File(
        "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/Revise-TotalEnergy/01_CreateDataset/SegmentedTest_Gasteiger_formation/SPICE-torsion-processed_charges-g-m_fullprecision_formationenergy.hdf5"
    )

    # Create the output file.
    filename = "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/Revise-TotalEnergy/01_CreateDataset/SegmentedTest_Gasteiger_formation/SPICE-torsion-processed_element-types_0-totalcharge_0-formalcharge_use-formationenergy_units-eV-A.extxyz"
    ...
    filter_molecular_charge = True
    allowed_charge = [0]

    filter_formal_charge = True
    ...
    for count in sorted(groupsByAtomCount.keys()):
        ...
        for g in groupsByAtomCount[count]:
            ...
            for i in range(numConfs):
                ...
                print(g["smiles"][i])
                curr_atoms.new_array("gasteiger_charges", g["gasteiger_charges"][i])
                curr_atoms.info["subset"] = g["smiles"][i].decode("utf-8")
