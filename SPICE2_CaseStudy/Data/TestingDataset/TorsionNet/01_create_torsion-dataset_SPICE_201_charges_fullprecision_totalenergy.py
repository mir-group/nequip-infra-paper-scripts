...
main_infile = h5py.File(
    "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/data/SPICE_TorsionNet500.hdf5"
)
# First pass: group the samples by total number of atoms.

groupsSplit = {}
groupsSplit["torsion"] = defaultdict(list)

for name in main_infile:
    group = main_infile[name]
    count = len(group["atomic_numbers"])
    groupsSplit["torsion"][count].append(group)
...
for split, groupsByAtomCount in groupsSplit.items():
    filename = "SPICE-torsion-processed_charges-g-m_fullprecision_formationenergy.hdf5"
    ...
    for count in sorted(groupsByAtomCount.keys()):
        ...
        group.create_dataset(
            "gasteiger_charges", data=np.array(q_array), dtype=np.float64
        )
