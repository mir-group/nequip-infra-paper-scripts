...
appearing_types = {}
infile = h5py.File(
    "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/data/SPICE-2.0.1.hdf5"
)
...
filename = "SPICE_2.0.1-processed_charges_fullprecision_totalenergy.hdf5"
...
for count in sorted(groupsByAtomCount.keys()):
    ...
    for g in groupsByAtomCount[count]:
        ...
        molTypes = [
            typeDict[(atom.symbol, atom.formal_charge.magnitude)] for atom in mol.atoms
        ]
        # print(molTypes)
        ...
        for i in range(numConfs):
            ...
            if "mbis_charges" in g.keys():
                ...
                print(f"At count: {count}, molecule {molSmiles} has no mbis charges")
