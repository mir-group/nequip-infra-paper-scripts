...
infile = h5py.File(
    "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/Revise-TotalEnergy/01_CreateDataset/SPICE_2.0.1-processed_charges_fullprecision_totalenergy.hdf5"
)

# Create the output file.
filename = "/lustre/orion/mat281/proj-shared/24_11_04_BigData/SPICE-2/Revise-TotalEnergy/01_CreateDataset/SPICE-2.0.1-processed_charges_element-types_no-2-atom_0-totalcharge_use-dfttotalenergy_units-eV-A.extxyz"
...
allowed_num_atoms = set(np.arange(1, 400))
allowed_num_atoms.remove(2)
print("allowed:", allowed_num_atoms)
...
for count in sorted(groupsByAtomCount.keys()):
    ...
    for g in groupsByAtomCount[count]:
        ...
        for i in range(numConfs):
            ...
            symbols = [reverseTypeDict[datasettype][0] for datasettype in g["types"][i]]
